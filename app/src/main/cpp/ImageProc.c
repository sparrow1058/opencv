...
extern void grayProc(unsigned char * cbuf,int w,int h);
...
void processimage (const void *p)
{
    ...
    yuyv422toABGRY((unsigned char *)p);
		grayProc((unsigned char *)lrgb,IMG_WIDTH,IMG_HEIGHT);      //leaf
		//LOGE("process image");
   // lrgb = &rgb[0];
}
...
void
Java_com_example_opencvtest_CameraPreview_pixeltobmp( JNIEnv* env,jobject thiz,jobject bitmap){
...
jint 
Java_com_example_opencvtest_CameraPreview_prepareCamera( JNIEnv* env,jobject thiz, jint videoid){
...
jint 
Java_com_example_opencvtest_CameraPreview_prepareCameraWithBase( JNIEnv* env,jobject thiz, jint videoid, jint videobase){
	
		int ret;

		camerabase = videobase;
	
		return Java_com_example_opencvtest_CameraPreview_prepareCamera(env,thiz,videoid);
	
}

void 
Java_com_example_opencvtest_CameraPreview_processCamera( JNIEnv* env,
										jobject thiz){
...
void 
Java_com_example_opencvtest_CameraPreview_stopCamera(JNIEnv* env,jobject thiz){
